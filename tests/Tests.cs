...
public class PartialReadTests
{
    ...
    public void TestStringArray(int bufferSize, string json)
    {
        ...
        var state = new Jacob.JsonReaderState();
        var jsonReader = JsonReader.Array2(JsonReader.String2());
        while (true)
        {
            WriteLine(new { Buffer = $"<{Encoding.UTF8.GetString(buffer)}>", buffer.Length });
            array = jsonReader.TryRead(ref reader, state);
    ...
    public void TestArrayOfStringArray(int bufferSize, string json)
    {
        ...
        var reader = new Utf8JsonReader(span, isFinalBlock: false, default);
        var state = new Jacob.JsonReaderState();
        var jsonReader = JsonReader.Array2(JsonReader.Array2(JsonReader.String2()));
        while (true)
        {
    ...
    public async Task TestArrayOfStringArrayAsync(int bufferSize, string json)
    {
        ...
        var jrs = new JsonReaderState();
        var bc = 0;
        var state = new Jacob.JsonReaderState();
        while (true)
        {
            var memory = await r.ReadAsync(bc, CancellationToken.None).ConfigureAwait(false);
            var array = Read();
            if (array.Incomplete)
                continue;
            Assert.Null(array.Error);
            ...
            {
                var reader = new Utf8JsonReader(memory.Span, memory.Length == 0, jrs);
                var chunk = Encoding.UTF8.GetString(memory.Span);
                WriteLine(new { Buffer = $"<{chunk}>", memory.Length });
                array = jsonReader.TryRead(ref reader, state);
                WriteLine($"BytesConsumed = {reader.BytesConsumed}");
                bc = (int)reader.BytesConsumed;
                jrs = reader.CurrentState;
                return array;
            }
        }
    }
